...
void D_Display (void)
{
    ...
    // BOURGEON PATCH: the melt wipe below runs as a SYNCHRONOUS do/while inside a
    // single doomgeneric_Tick (I_Sleep(1) + repeated DG_DrawFrame) — a 0.5-1.5 s
    // freeze of the host game's render thread, and invisible anyway since the host
    // presents only once per frame. Skip it entirely.
    wipe = false;
...
static void D_Endoom(void)
{
    ...
    // BOURGEON PATCH: embedded build — this atexit handler runs on the in-game
    // Quit path (I_Quit atexit chain); exit(0) here would kill the host RO client.
    // I_Endoom is already a no-op in this build; just return.
    /* exit(0); */
}
