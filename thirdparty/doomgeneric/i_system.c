...
void I_Quit (void)
{
    ...
    // BOURGEON PATCH: embedded build — no exit(). Tell the plugin the user quit
    // DOOM (menu Quit -> 'y'); it stops ticking and hides the window. The engine
    // is a zombie past this point (atexit chain ran) and must not be ticked again.
    {
        extern void DG_OnDoomQuit(void);
        DG_OnDoomQuit();
    }
}
...
void I_Error (char *error, ...)
{
    ...
    exit(-1);
#else
    // BOURGEON PATCH: we run embedded in a game process — exit() would kill the
    // host client. Hand control back to the plugin, which longjmp()s out of the
    // engine and marks DOOM dead (see doom_tweaks.cc). Never returns.
    {
        extern void DG_OnFatalError(const char* msg);
        DG_OnFatalError(msgbuf);
    }
    exit(-1);  // unreachable fallback (jump buffer is always armed)
#endif
}
