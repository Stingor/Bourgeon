...
{
    // BOURGEON PATCH: embedded build — the host CWD is the RO client folder;
    // configdir "." would scatter ".default.cfg" / ".savegame/" there (filenames
    // are built with a separator-less M_StringJoin, hence the trailing slash).
    // Keep everything in a "doom" subfolder instead (M_SetConfigDir mkdir's it).
