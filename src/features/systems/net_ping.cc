...
constexpr uint64_t kMaxRoundTripMs = 1500;
...
std::atomic<int>      NetPing::s_last_ms_{-1};
...
void NetPing::NoteSend(uint16_t opcode, int packet_len) {
  ...
  s_sent_at_.store(NowMs(), std::memory_order_release);
}
...
void NetPing::OnRecvPacket(uint16_t opcode, const uint8_t*, uint16_t) {
  ...
  const uint64_t rtt = (now > sent) ? (now - sent) : 0;
  if (rtt > kMaxRoundTripMs) return;
  ...
  s_last_ms_.store(static_cast<int>(rtt), std::memory_order_relaxed);
}
...
void NetPing::OnModeSwitch(ModeMgr::ModeType mode_type, const char*) {
  ...
  s_last_ms_.store(-1, std::memory_order_relaxed);
}

int NetPing::LastMs() { return s_last_ms_.load(std::memory_order_relaxed); }
