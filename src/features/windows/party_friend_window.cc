...
bool PartyFriendWindow::DrawSettings() {
  ...
  if (show_buffs_) {
    // Le bloc est commun avec la grille de groupe (features/status_cell.h) ;
    // seules les bornes du curseur de taille nous sont propres — une ligne de
    // liste porte des icônes plus grandes qu'une tuile de grille.
    changed |= statuscell::DrawSettings({&buff_preview_, &buff_px(), &buff_max(),
                                         &buff_rows(), &buff_time_, &buff_sweep()},
                                        /*size_min_px=*/8, /*size_max_px=*/32);
  }
