...
void DrawDebugControls() {
  ...
  if (g_resolved_concrete > 0)
    ...
    ImGui::TextColored(ImVec4(1.0f, 0.5f, 0.3f, 1.0f), "%s", i18n::Tr("-> non résolu (en jeu ? Lua prêt ?)"));
  if (ImGui::IsItemHovered())
    ImGui::SetTooltip("%s", i18n::Tr("L'id concret vient de GetHatEffectID(ordinal), le getter du jeu.\n"
  ...
  if (ImGui::IsItemHovered())
    ImGui::SetTooltip("%s", i18n::Tr("Les instances CEZ2STREffect sont normalement écartées (autre pipeline).\n"
  ...
  if (ImGui::IsItemHovered())
    ImGui::SetTooltip("%s", i18n::Tr("Sert à isoler une régression : coche, puis regarde si le rendu NATIF\n"
  ...
  if (on) {
    ...
    ImGui::SameLine();
    if (st.draws == 0)
      ImGui::TextColored(ImVec4(1.0f, 0.55f, 0.3f, 1.0f), "%s", i18n::Tr("-> nœud jamais dessiné (non créé ?)"));
    else if (st.inserts == 0)
      ImGui::TextColored(ImVec4(1.0f, 0.8f, 0.3f, 1.0f), "%s", i18n::Tr("-> dessiné mais n'émet RIEN (ressource ?)"));
    else if (n_ours == 0)
      ImGui::TextColored(ImVec4(1.0f, 0.4f, 0.4f, 1.0f), "%s", i18n::Tr("-> émis mais REJETÉ par nous (bug)"));
    else
      ImGui::TextColored(ImVec4(0.4f, 1.0f, 0.5f, 1.0f), "%s", i18n::Tr("-> OK"));
    ...
    if (ImGui::IsItemHovered())
      ImGui::SetTooltip("%s", i18n::Tr("dessins  = appels d'EzEffect_Draw sur NOTRE nœud\n"
  ...
  // Placé HAUT dans le panneau, au-dessus du catalogue, pour rester visible sans défiler.
  {
    ...
    if (nv > 0) {
      ...
      ImGui::TextDisabled("%s", i18n::Tr("ids CEffectMgr : aucun"));
    }
    if (ImGui::IsItemHovered())
      ImGui::SetTooltip("%s", i18n::Tr("Valeurs BRUTES lues à instance+0x04, pour établir leur encodage :\n"
    ...
    if (na > 0) {
      ...
      if (ImGui::IsItemHovered())
        ImGui::SetTooltip("%s", i18n::Tr("Tous les ids présents dans la capture, au format id×nombre.\n"
      ...
      ImGui::TextDisabled("%s", i18n::Tr("effect_id capturés : aucun (rien n'est capturé)"));
    }
  }
  ...
  // L'adresse qui n'apparaît QUE lorsque l'effet tourne est la fonction cherchée.
  {
    ...
    if (ImGui::IsItemHovered())
      ImGui::SetTooltip("%s", i18n::Tr("Adresses de retour des fonctions qui soumettent des primitives, avec leur\n"
    ...
    if (show_callers) {
      ...
      if (ImGui::BeginChild("##ez_callers", ImVec2(0, 0), true)) {
        ...
        if (n == 0) ImGui::TextDisabled("%s", i18n::Tr("(aucun appel cette frame)"));
      }
      ImGui::EndChild();
    }
  }
  ...
  if (ImGui::IsItemHovered())
    ImGui::SetTooltip("%s", i18n::Tr("Natif par primitive : rejoue SRCBLEND/DESTBLEND que le jeu a posés pour\n"
  ...
  if (bl_n > 0) {
    ...
    ImGui::TextDisabled("%s", i18n::Tr("blends: -"));
  }

  ro::RoCheckbox(i18n::Tr("Debug capture (brut + ancre)"), &g_debug);
  if (ImGui::IsItemHovered())
    ImGui::SetTooltip("%s", i18n::Tr("Dessine la capture BRUTE (positions écran natives) + l'ancre (point cyan) et\n"
  ...
  if (ImGui::IsItemHovered())
    ImGui::SetTooltip("%s", i18n::Tr("Garde-fou large par défaut (la géométrie « traînée » est LÉGITIME :\n"
  ...
  if (g_catalog_built) {
    ...
    if (ImGui::IsItemHovered())
      ImGui::SetTooltip("%s", i18n::Tr("Un effect id sans entrée dans la table de saut du client (0x00bc2e04)\n"
    ...
    ImGui::EndChild();
    ImGui::TextDisabled("%s", i18n::Tr("« inerte » = ni entrée procédurale ni resourceFileName -> ne rend rien, même"));
    ImGui::TextDisabled("%s", i18n::Tr("nativement.  « .str » = rend en jeu, mais via le pipeline billboard STR :"));
    ImGui::TextDisabled("%s", i18n::Tr("un autre chemin que EzEffect_Draw, donc a priori NON capturé par ce lab."));
  }
