...
// InventoryViewer — la fenêtre chariot EST une sœur de l'inventaire dans le
...
// imgui_enabled_, qui n'est PAS un réglage isolé : le chariot fait partie du lot
// « Interface moderne » (SetModernInterface) avec l'inventaire, le storage, les
// barres d'action, l'échange et le courrier. Un chariot moderne qui devrait
// échanger des items par glisser avec un inventaire natif n'aurait aucun sens.
//
// Lecture du modèle : liste SESSION du chariot (g_session+0x1720 = 0x015fbae0,
...
// (@tri_cart / réglage « Tri Chariot ») est reflété dès que le serveur renvoie la
...
// UICartWnd_OnMsg case 38) : chariot -> inventaire 0x4d, chariot -> storage 0x4f,
// inventaire -> chariot 0x4c, storage -> chariot 0x4e.

class CartViewer : public Plugin {
 public:
  const char* name() const override { return "CartViewer"; }

  void OnTick() override;      // état du chariot (polling read-only) + masquage natif
  void OnRenderUI() override;  // grille ImGui si le chariot est ouvert
  ...
  bool imgui_enabled_ = false;  // OPT-IN : chariot natif par défaut

  // ── Settings PERSISTANTS (section « Chariot » du panneau Moonlight) ─────────
  ...
  // True si (mx,my) est au-dessus du viewer chariot ouvert. Sert aux AUTRES viewers
  // (inventaire, storage) pour router un dépôt par glisser : quand le chariot est
 ...
 private:
  // Un item du chariot, extrait en POD (sous SEH) pour un rendu hors __try.
  struct Item {
    uint32_t id = 0;          // atoi(std::string @info+0x2c)
    int      amount = 0;      // node+0x18 (= info+0x10)
    int      index = 0;       // info+4 : index chariot (arg des commandes de transfert)
  ...
  int  pend_index_ = 0;   // index chariot de l'item concerné
  ...
  bool open_ = false;      // chariot ouvert ce frame ?
