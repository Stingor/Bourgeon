...
#include "plugins/inventory_viewer.h"  // PointOverViewer (dépôt vers le viewer inventaire)
...
void CartViewer::OnRenderUI() {
                    ...
                    ImGuiWindowFlags_AlwaysVerticalScrollbar);
  {
    ...
    for (int k = 0; k < nitems; ++k) {
      ...
      // Icône à sa taille NATIVE, centrée ; réduite seulement si plus grande que la tuile.
      ...
      if (ic.tex && ic.w > 0 && ic.h > 0) {
        ...
        dl->AddImage(TexId(ic.tex), ip, ImVec2(ip.x + dw, ip.y + dh),
                     ImVec2(0, 0), ImVec2(1, 1), SkinImgTint());
      } else {
      ...
      // Coin bas-droit : refine « +N » (équipement) OU quantité (pile) — exclusifs.
      // Texte NOIR cerné de BLANC (lisible sur tout fond), comme le natif.
      char q[16] = {0};
