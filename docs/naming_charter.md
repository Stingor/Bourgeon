...
Le chantier 8 (tables de descripteurs pour la persistance) avait déjà supprimé une grande partie du code décrit ici avant que le renommage ne commence : `moonlight_ui.cc` est passé de 3542 à ~1230 lignes, et `LoadSettings` de 516 à 70. Les items **#3 (`p`), #4 (`load_color`/`load_col`), #8 (`c`), #9 (`g`), #10 (`s_iface_nav`), #11, #12, #14 (`hex`)** ainsi que le **gate mort de la section Chat** et l'**étape 0** entière sont donc morts d'eux-mêmes, sans renommage : la fonction qui les hébergeait n'existe plus.

Ont été appliqués : **#1** (`eb`), **#2** (alias de plugins frères), **#5** (bascules serveur), **#6** (noms Ghidra), **#7** (parsing réseau), **#13** (`AlootPreset::no` + enum), **#15** (`namespace mui`), les noms trompeurs survivants (`kSettingStaff`, `in_gonryun_`, `AlootPreset::no`, `static char hdr`, `IsHovered`, `ColorPicker`, `AlignGrid::cell()`, la lambda `preset` de basic_info, `vtable_ptr`, `enriched_item`, `base`), et les défauts cités en exemple des règles hors `moonlight_ui` (R4 `hookAddr`, R5 `leftHand`/`posX`, R6 `kcPose`/`keInvIndex`, R8 `param_1`, R14 paramètres anonymes).

**Reste identifié, non traité** — hors périmètre de l'audit d'origine : six autres hooks d'`OnMsg` natif (`basic_info.cc`, `equip_tweaks.cc`, `status_tweaks.cc`, `inventory_tweaks.cc`, `chat.cc`, `char_select.cc`) portent encore des paramètres `p1`/`p3`..`p6`, même défaut R8. Les nommer demande de savoir ce que chacun transporte dans CE message-là ; un nom faux y serait pire que `p3`.

## État des lieux (au moment de l'audit)
