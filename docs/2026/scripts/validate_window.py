# -*- coding: utf-8 -*-
"""Valide l'appariement par co-occurrence d'ids, DEUX tests independants.
...
a25 = json.load(open(os.path.join(D, 'annot_2025_win.json')))
a26 = json.load(open(os.path.join(D, 'annot_2026_win.json')))
...
for src, rec in pairs['functions'].items():
    ...
    if n25 == n26:
        same += 1
        examples.append((src, rec['to'], n25, rec['nid']))
    else:
        diff += 1
        mismatches.append((src, rec['to'], n25, n26, rec['nid']))
...
for s, t, n, k in examples[:12]:
    print("    OK  %s -> %s  %s  (%d ids)" % (s, t, n, k))
for s, t, x, y, k in mismatches[:12]:
    print("    !!  %s -> %s  %s  vs  %s  (%d ids)" % (s, t, x, y, k))
...
# ---------------------------------------------------- qualite par nb d'ids
print("\n=== ratio de tailles selon la force de la signature ===")
for lo, hi, label in ((3, 999, ">= 3 ids"), (2, 2, "2 ids"), (1, 1, "1 opcode")):
    sub = []
    for src, rec in pairs['functions'].items():
        if not (lo <= rec['nid'] <= hi):
            continue
        s1 = a25['functions'].get(norm(src), {}).get('size', 0)
        s2 = a26['functions'].get(norm(rec['to']), {}).get('size', 0)
        r = ratio(s1, s2)
