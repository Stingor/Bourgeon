...
- Les réglages d'un plugin vivent dans le `DrawSettings()` de ce plugin, jamais inlinés dans moonlight_ui. moonlight_ui n'est qu'une navigation : `if (auto* p = …) changed = p->DrawSettings(); else GrayText(kUnavailableText);`.
